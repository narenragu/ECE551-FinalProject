  ...
  task automatic monitor_maze_solve();
    fork
        ...
        forever @(posedge clk) begin
            ...
            if (iDUT.iSLV.strt_mv)
                $display("MAZE_SOLVE: strt_mv (stp_lft=%b stp_rght=%b) at %0t",
                         iDUT.iSLV.stp_lft, iDUT.iSLV.stp_rght, $time);
            if (iDUT.sol_cmplt)
                $display("MAZE_SOLVE: sol_cmplt! at %0t", $time);
        end

        /*
        // Monitor position changes
        forever @(posedge clk) begin
            if ($unsigned(iPHYS.xx - prev_xx) > 15'h20 ||
                $unsigned(iPHYS.yy - prev_yy) > 15'h20) begin
                $display("POSITION: xx=0x%h yy=0x%h heading=%0h at %0t",
                         iPHYS.xx, iPHYS.yy, iPHYS.heading_robot[19:8], $time);
                prev_xx = iPHYS.xx;
                prev_yy = iPHYS.yy;
            end
        end
        */
    join_none  // don't block — run in background
  endtask
